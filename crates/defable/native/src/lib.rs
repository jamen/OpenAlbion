...
fn launch_fable(mut cx: FunctionContext) -> JsResult<JsUndefined> {
    ...
    let dll_path_original = CString::new("C:\\Users\\Jamen\\Repos\\defable\\target\\i686-pc-windows-msvc\\debug\\fable_cheat.dll").unwrap();
    ...
    let dll_path_in_remote = unsafe {
        VirtualAllocEx(hprocess, null_mut(), dll_path_original.to_bytes().len(), MEM_RESERVE | MEM_COMMIT, PAGE_EXECUTE_READWRITE)
    };

    println!("dll_path_in_remote {:?} {:?}", dll_path_in_remote, dll_path_original.to_bytes().len());

    unsafe {
        WriteProcessMemory(hprocess, dll_path_in_remote, dll_path as LPVOID, dll_path_original.to_bytes().len(), null_mut())
    };
